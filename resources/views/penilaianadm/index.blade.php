        ...
        <div class="row">
          <div class="col-md-4">
            <select name="jenis" class="form-select">
              ...
              <option value="komprehensif" {{ $jenis == 'komprehensif' ? 'selected' : '' }}>Komprehensif</option>
            </select>
          </div>
          <div class="col-md-2">
      ...
      ">
          <thead style="background-color:#1e3a8a; color:#ffffff;">
              <tr>
                  ...
                  <th style="padding:12px 14px; text-align:left; font-size:14px;">Rata-rata Nilai</th>
            ...
            @foreach($jadwal as $item)
              <tr style="background-color:#ffffff;">
                  ...
                  <td style="padding:12px 14px; font-size:14px;">
                    @if($item->rata_rata_nilai)
                      <span style="font-weight:bold; color:#2563eb;">{{ $item->rata_rata_nilai }}</span>
                    @else
                      -
                    @endif
                  </td>
                  <td style="padding:12px 14px;">
                      @if($item->all_penilaian->isNotEmpty())
                        <span style="padding:4px 10px;border-radius:12px;font-size:12px;font-weight:bold;background:#bbf7d0;color:#166534; display:inline-block;">
                          Sudah Dinilai
                        </span>
                      @else
                        <span style="padding:4px 10px;border-radius:12px;font-size:12px;font-weight:bold;background:#fde68a;color:#92400e; display:inline-block;">
                          Belum Dinilai
                        </span>
                      @endif
                  </td>
                  <td style="padding:12px 14px;">
                    @if($item->all_penilaian->isNotEmpty())
                      <a href="{{ route('penilaianadm.show', $item->id) }}">
