    ...
    <section class="guest-artikel-detail-section container my-5">
        <div class="row">
            ...
            <p class="guest-galery-desc">
                Profil alumni ini menjadi pengingat bahwa keberhasilan hadir dalam banyak bentuk, dengan satu semangat yang
                sama: berkembang dan memberi makna.
            </p>
            ...
            <div class="guest-galery-pagination d-flex align-items-center justify-content-center mt-4">
                ...
                <div class="guest-galery-pages d-flex">
                    @foreach ($reviews->getUrlRange(1, $reviews->lastPage()) as $page => $url)
                        <a href="{{ $url }}"
                            class="guest-galery-page {{ $page == $reviews->currentPage() ? 'active' : '' }}"
                            style="background-color: {{ $page == $reviews->currentPage() ? '#FFD700' : '#eaeaea' }}; color: {{ $page == $reviews->currentPage() ? '#fff' : '#333' }}; padding: 6px 12px; border-radius: 6px; margin: 0 4px; font-weight: 600; text-decoration: none; transition: 0.2s;">{{ $page }}</a>
                    @endforeach
                </div>
