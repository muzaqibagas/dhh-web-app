      ...
      <main class="guest-artikel-main h-100">
        ...
        @else        
          <div class="guest-artikel-featured-grid">
            ...
            <a href="{{ route('artikel.show', $item->id) }}" class="guest-artikel-featured-card text-decoration-none text-dark">
              <img src="{{ asset($item->foto) }}" alt="featured">
              <div class="guest-artikel-featured-meta">                
                <span class="guest-artikel-badge" style="background-color: {{ $item->kategoriartikel->getBadgeColor() }}; color: #fff;">{{ $item->kategoriartikel->nama ?? '-' }}</span>
                <span class="guest-artikel-date">{{ \Carbon\Carbon::parse($item->tanggal)->format('d M Y') }}</span>
                <h3>{{ $item->judul }}</h3>
              </div>
            </a>
            @endforeach        
          ...
          <div class="guest-artikel-columns">            
            ...
            <div class="guest-artikel-list-col">
              ...
              <a href="{{ route('artikel.show', $item->id) }}" class="guest-artikel-list-item text-decoration-none text-dark">
          ...
          <div class="guest-galery-pagination d-flex align-items-center justify-content-center mt-4">
              ...
              <div class="guest-galery-pages d-flex">
                  @foreach ($pagination->getUrlRange(1, $pagination->lastPage()) as $page => $url)                      
                      <a href="{{ $url }}" class="guest-galery-page {{ $page == $pagination->currentPage() ? 'active' : '' }}" style="background-color: {{ $page == $pagination->currentPage() ? '#FFD700' : '#eaeaea' }}; color: {{ $page == $pagination->currentPage() ? '#fff' : '#333' }}; padding: 6px 12px; border-radius: 6px; margin: 0 4px; font-weight: 600; text-decoration: none; transition: 0.2s;">{{ $page }}</a>
                  @endforeach
              </div>
