        ...
        <main class="content">
            <div class="container-fluid mt-4">
                <div class="adm-header">
                    @php
                        $labelUjian = match ($syaratUjian->jenis_ujian) {
                            'kolokium' => 'Kolokium',
                ...
                <div class="card shadow-sm">
                    <div class="card-body bg-light">
                        ...
                        <div class="ratio ratio-16x9 mb-3" style="min-height:350px;">
                            @if (strtolower($ext) === 'pdf')
                                <iframe src="{{ asset($formulirPath) }}"
                                    style="border-radius:8px; border:1px solid #ddd; min-height:350px;" allowfullscreen>
                                </iframe>
                            @endif
                        </div>

                        <form action="{{ route('syaratujian.tambahModerator', $syaratUjian->id) }}" method="POST"
                            class="row g-3">
                            @csrf
                            ...
                            <div class="d-flex flex-column gap-3">
                                <div class="col-12 col-md-8 d-flex align-items-center">
                                    @php
                                        $labelModerator =
                                            $syaratUjian->jenis_ujian === 'komprehensif' ? 'Ketua Sidang' : 'Moderator';
                                    @endphp

                                    <label for="moderator" class="form-label mb-0 w-50">Nama Dosen
                                        {{ $labelModerator }}</label>
                                    ...
                                    <select name="moderator" id="moderator" class="form-control" required>
                                        ...
                                        @foreach ($listModerator as $dosen)
                                            <option value="{{ $dosen->id }}"
                                                {{ $syaratUjian->id_moderator == $dosen->id ? 'selected' : '' }}>
                                                {{ $dosen->nama }}
                                            </option>
                                        @endforeach
                                    </select>
                                ...
                                <div class="col-12 col-md-8 d-flex align-items-center">
                                    <label for="penandatanganundangan" class="form-label mb-0 w-50">Nama Penandatangan
                                        Undangan</label>
                                    <span style="margin-right: 8px;">: </span>
                                    <select name="penandatanganundangan" id="penandatanganundangan" class="form-control"
                                        required>
                                        <option value="">-- Pilih Penandatangan Undangan --</option>
                                        @foreach ($listModerator as $dosen)
                                            <option value="{{ $dosen->id }}"
                                                {{ $syaratUjian->id_penandatanganundangan == $dosen->id ? 'selected' : '' }}>
                                                {{ $dosen->nama }}
                                            </option>
                                        @endforeach
                                    </select>
                                ...
                                @if ($syaratUjian->jenis_ujian === 'komprehensif')
                                    <div class="col-12 col-md-8 d-flex align-items-center">
                                        <label for="penguji" class="form-label mb-0 w-50">Nama Dosen Penguji</label>
                                        <span style="margin-right: 8px;">: </span>
                                        ...
                                        <select name="penguji" id="penguji" class="form-control" required>
                                            ...
                                            @foreach ($penguji as $dosen)
                                                <option value="{{ $dosen->id }}"
                                                    {{ $syaratUjian->id_penguji == $dosen->id ? 'selected' : '' }}>
                                                    {{ $dosen->nama }}
                                                </option>
                                            @endforeach
                                        </select>
                                    ...
                                    <div class="col-12 col-md-8 d-flex align-items-center">
                                        ...
                                        <input type="text" name="ruangan" id="ruangan" class="form-control"
                                            value="{{ old('ruangan', $syaratUjian->ruangan) }}"
                                            placeholder="Contoh: Ruang Sidang Lt. 2 / Zoom" required>
                                    </div>
                                @endif
    ...
    @push('script')
        ...
        <script>
            $(document).ready(function() {
                ...
                // Select2 untuk pilihan Penandatangan Undangan
                ...
                // Select2 untuk penguji hanya jika elemen ada
                if ($('#penguji').length) {
                    $('#penguji').select2({
                        width: '100%',
