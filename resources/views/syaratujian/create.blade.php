      ...
      <div class="syarat-card">
          ...
          @if($syarat && $syarat->status === 'disetujui')
            <div class="alert alert-success">
              Dokumen Anda sudah <b>disetujui</b>. Anda tidak bisa upload lagi, silahkan melaksanakan {{ $label }}.
            </div>
          ...
          @elseif($syarat && $syarat->status === 'ditolak')
            <div class="alert alert-warning">
              ...
              <ul>
                @if($syarat->alasan_formulir)<li><b>Formulir {{ $label }}:</b> {{ $syarat->alasan_formulir }}</li>@endif
                @if($syarat->alasan_makalah)<li><b>Makalah {{ $label }}:</b> {{ $syarat->alasan_makalah }}</li>@endif
                ...
                @if($syarat->alasan_bukti_kehadiran)<li><b>Bukti Kehadiran {{ $label }}:</b> {{ $syarat->alasan_bukti_kehadiran }}</li>@endif
              </ul>
            </div>
            ...
            <div class="upload-section">
              ...
              <form action="{{ route('syaratujian.reupload', $syarat->id) }}" method="POST" enctype="multipart/form-data">
                @csrf
                <input type="hidden" name="jenis_ujian" value="{{ $jenis }}">

                @if($syarat->alasan_formulir)
                  <div class="form-group">
                    <label>Upload Ulang Formulir {{ $label }} <small class="text-danger">(*format wajib .PDF)</small></label>
                    <input type="file" name="formulir" accept=".pdf" required>
                  </div>
                @endif
                @if($syarat->alasan_makalah)
                  <div class="form-group">
                    <label>Upload Ulang makalah {{ $label }} <small class="text-danger">(*format wajib .PDF)</small></label>
                ...
                @if($syarat->alasan_bukti_kehadiran)
                  <div class="form-group">
                    <label>Upload Ulang Bukti Kehadiran {{ $label }} <small class="text-danger">(*format wajib .PDF)</small></label>
          ...
          @elseif ($syarat && $syarat->bap === 'ditolak' && !$syarat->formulir)
            <div class="alert alert-warning">
              <strong>BAP Anda ditolak.</strong> Silakan unggah ulang<strong>Formulir {{ $label }}</strong> dengan jadwal baru untuk penjadwalan ulang.
              <ul>
                @if($syarat->alasan_formulir)<li><b>Formulir {{ $label }}:</b> {{ $syarat->alasan_formulir }}</li>@endif                
              </ul>
            </div>        
          ...
          @else
            <div class="upload-section">
              ...
              <form action="{{ route('syaratujian.store') }}" method="POST" enctype="multipart/form-data">
                ...
                <div class="form-group">
                  <label>Upload Formulir Pendaftaran {{ $label }} <small class="text-danger">(*format wajib .PDF)</small></label>
                  <input type="file" name="formulir" accept=".pdf" required>
                </div>

                <div class="form-group">
                  <label>Upload Makalah {{ $label }} <small class="text-danger">(*format wajib .PDF)</small></label>
                  <input type="file" name="makalah" accept=".pdf" required>
                </div>
                ...
                <div class="form-group">
                  <label>Upload Bukti Kehadiran {{ $label }} <small class="text-danger">(*format wajib .PDF)</small></label>
                  <input type="file" name="bukti_kehadiran" accept=".pdf" required>
                </div>
