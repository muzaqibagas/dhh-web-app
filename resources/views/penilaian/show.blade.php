...
@section('content')
    ...
    <div class="main-container">
            ...
            <div class="card text-start mb-4 p-4" style="background-color:#f3f4f6;">
                ...
                <table class="info-table">
                    ...
                    <tr>
                        <td>Nilai Akhir</td>
                        <td>:</td>
                        <td>{{ $penilaians->first()?->nilai_akhir !== null ? number_format($penilaians->first()->nilai_akhir, 2) : '-' }}
                        </td>
                    </tr>
                </table>
            </div>
            <div class="card text-start mb-4 p-4">
                <h2 style="margin-top:0; font-size:18px; color:#1e3a8a;">Hasil Penilaian</h2>
                @if ($penilaians->isEmpty())
                    <div style="padding:16px; background:#f8d7da; color:#842029; border-radius:8px; margin-bottom:16px;">
                        Tidak ada data penilaian untuk ditampilkan.
                    </div>
                @else
                    <table
                        style="
        ...
        ">
                    ...
                    <div
                        style="margin-top:16px; margin-bottom:16px; padding:14px; background:#eef2ff; border-radius:8px; font-size:14px;">
                        <strong>Total Nilai Akhir:</strong>
                        {{ $penilaians->first()?->nilai_akhir !== null ? number_format($penilaians->first()->nilai_akhir, 2) : '-' }}
                    </div>

                    @if ($penilaians->first()?->catatan)
                  ...
                  font-size:14px;
                ">
                                {{ $penilaians->first()->catatan }}
                            </div>
                        </div>
                    @endif
                @endif

                <h2 style="margin-top:50px; font-size:18px; color:#1e3a8a;">Rekap Nilai Dosen</h2>
                <table
                    style="
          width:100%;
          border-collapse:collapse;
        ...
        ">
                    <thead style="background:#1e3a8a; color:white;">
                        <tr>
                            ...
                            <th style="padding:10px;">Jenis Ujian</th>
                            <th style="padding:10px;">Peran</th>
                            <th style="padding:10px;">Nilai Akhir</th>
                        </tr>
                    </thead>
                    <tbody>
                        @foreach ($nilaiPerDosen as $i => $item)
                            <tr>
                                ...
                                <td style="padding:10px;">{{ $item['nama_dosen'] }}</td>
                                <td style="padding:10px;">{{ ucfirst($jenis) }}</td>
                ...
                <div style="margin-top:16px; padding:14px; background:#dcfce7; border-radius:8px; font-size:14px;"
          ...
          margin-top:20px;">
                    <a href="{{ route('dashboarddosen.index') }}"
                        style="
