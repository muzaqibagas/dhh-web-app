...
@section('content')
    ...
    <section class="guest-galery-section">
        ...
        <div class="guest-galery-video">
            @if ($videoTerbaru)
                <iframe width="100%" height="500" src="{{ $videoTerbaru->video }}" title="{{ $videoTerbaru->judul }}"
                    frameborder="0"
        ...
        <div class="guest-galery-videos">
            @foreach ($videoLainnya as $video)
                <div class="guest-galery-video-card">
                    <iframe width="100%" height="200" src="{{ $video->video }}" frameborder="0"
                        allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture"
                        allowfullscreen>
                    </iframe>
                </div>
        ...
        <p class="guest-galery-desc">
            Sebagaimana hutan menyimpan jejak kehidupan, galeri ini menyimpan jejak langkah kami dalam berkegiatan, belajar,
            dan tumbuh bersama.
        </p>
        ...
        <ul class="guest-galery-filters">
            ...
            @foreach ($kategoriGaleri as $kategori)
                <li class="guest-galery-filter {{ isset($kategoriAktif) && $kategoriAktif == $kategori->id ? 'active' : '' }}"
        ...
        <div class="guest-galery-pagination d-flex align-items-center justify-content-center mt-4">
            ...
            <div class="guest-galery-pages d-flex">
                @foreach ($galeriFoto->getUrlRange(1, $galeriFoto->lastPage()) as $page => $url)
                    <a href="{{ $url }}"
                        class="guest-galery-page {{ $page == $galeriFoto->currentPage() ? 'active' : '' }}"
                        style="background-color: {{ $page == $galeriFoto->currentPage() ? '#FFD700' : '#eaeaea' }}; color: {{ $page == $galeriFoto->currentPage() ? '#fff' : '#333' }}; padding: 6px 12px; border-radius: 6px; margin: 0 4px; font-weight: 600; text-decoration: none; transition: 0.2s;">{{ $page }}</a>
                @endforeach
            </div>
